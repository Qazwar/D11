...
namespace ds {	
	...
	struct EdgeConnection {
		...
		bool contains(uint16_t edge_index) const {
			return first == edge_index || second == edge_index;
		}

		uint16_t opposite(uint16_t edge_index) const {
			if (first == edge_index) {
				return second;
			}
			else {
	...
	public:
		...
		int find_connections(uint16_t face_index, EdgeConnection* connections, int max);
